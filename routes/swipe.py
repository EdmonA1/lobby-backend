...
async def like_user(body: LikeCreate, user=Depends(get_current_user)):
    ...
    # ✅ ИСПРАВЛЕНИЕ: Проверяем матч ПОСЛЕ сохранения лайка
    they_liked_me = has_liked(body.to_user, my_id)
    is_new_match = False
    match_user = None

    if they_liked_me:
        if not match_exists(my_id, body.to_user):
            # ✅ Новый матч
        ...
        # ✅ Матч уже существует — не показываем снова
        # is_new_match остаётся False

    remaining = (
        max(0, FREE_LIKES_PER_DAY - user["likes_today"])
        if not user.get("premium") else -1
    )

    return {
        "match": is_new_match,        # ✅ Только НОВЫЕ матчи
        "match_user": match_user,     # ✅ Только если новый
