...
from utils.storage import get_user, save_user, add_like, has_liked, add_match, match_exists
...
async def like_user(body: LikeCreate, user=Depends(get_current_user)):
    ...
    save_user(user)
    is_match = has_liked(body.to_user, user["telegram_id"])
    match_user = None
    if is_match and not match_exists(user["telegram_id"], body.to_user):
        ...
        try:
            ...
            pass
    remaining = max(0, FREE_LIKES_PER_DAY - user["likes_today"]) if not user.get("premium") else -1
    return {"match": is_match, "match_user": match_user, "likes_remaining": remaining}
